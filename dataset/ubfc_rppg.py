    ...
    def preprocess(self, frames, gts):
        ...
        for data_type in self.config.DATA_TYPE:
            ...
            if data_type == "Raw":
                ...
                x.append(utils.standardized_data(f_c)[:-1, :, :, :])
            else:
        ...
        if self.config.LABEL_TYPE == "Raw":
            ...
            y[0, :] = utils.standardized_label(gts[0, :])[:-1]
        else:
...
class UBFC_rPPG(data.Dataset):
    def __init__(self, config):
        super(UBFC_rPPG, self).__init__()
        self.config = config
    ...
    def __getitem__(self, idx):
        ...
        y = torch.from_numpy(np.load(y_path))  # T x 3
        return x, y
